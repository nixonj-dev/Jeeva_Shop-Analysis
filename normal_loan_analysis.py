...
def render(data, ao_data, outer_data=None, outer_closed_data=None):
    ...
    if sub_page in ["💰 Amount & Growth", "📑 All Summary (Everything)"]:
        ...
        # Added Day of Week, Loan Volume, and Future Analysis to these tabs!
    ...
    if sub_page in ["⚠️ Active & Overdue", "📑 All Summary (Everything)"]:
        ...
        if ao_data is not None and not ao_data.empty:
            ...
            if not f_ao.empty:
                # --- NEW: Map Outer Amounts to the Live Loans ---
                if outer_data is not None and not outer_data.empty:
                    outer_map = outer_data.groupby('form_no')['amount'].sum().to_dict()
   ...
   # ==========================================
    ...
    # ==========================================
        # 🧠 FEED DATA TO THE AI ASSISTANT
        # ==========================================
        try:
            t_disp = filtered_data['total_given_amount'].sum() if not filtered_data.empty else 0
            t_act = ao_data[ao_data['status'].str.lower() == 'active']['present_amount'].sum() if not ao_data.empty else 0
            t_ovr = ao_data[ao_data['status'].str.lower() == 'overdue']['present_amount'].sum() if not ao_data.empty else 0
            
            st.session_state['ai_context'] = f"""
            --- CURRENT PORTFOLIO DATA ---
            Total Dispersal on this page: ₹ {t_disp:,.0f}
            Safe Active Capital: ₹ {t_act:,.0f}
            Risk/Overdue Capital: ₹ {t_ovr:,.0f}
            
            (Note: If the user asks for exact Gold vs Silver dispersal numbers, advise them to navigate to the 'Master Final Report' tab where those advanced metrics are calculated.)
            """
        except Exception:
            pass